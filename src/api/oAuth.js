...
export const OAuthURL = {
  ...
  naver: `https://nid.naver.com/oauth2.0/authorize?client_id=${NAVER_CLIENT_ID}&redirect_uri=${NAVER_REDIRECT_URL}&response_type=token&state=state`,
  google: ``,
}
  ...
  sendKakaoCodeToServer: async code => {
    ...
    try {
      response = await axiosInstance({
        url: `/oauth2/kakao/login?code=${code}`,
        method: "GET",
      })
  ...
  sendNaverCodeToServer: async code => {
    try {
      const response = await axiosInstance({
        url: `/login/oauth2/naver?code=${code}`,
        method: "GET",
      })
      console.log(response)
    } catch (err) {
